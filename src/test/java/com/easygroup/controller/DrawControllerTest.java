...
class DrawControllerTest {
        ...
        void testCompleteWorkflow_PreviewThenSave() throws Exception {
                ...
                System.out.println("Request: POST /api/lists/" + listId + "/draws?save=false");
                ...
                System.out.println("   Balance by Gender: " + generateRequest.getBalanceByGender());
                System.out.println("   Balance by Age: " + generateRequest.getBalanceByAge());
                System.out.println("   Balance by French Level: " + generateRequest.getBalanceByFrenchLevel());
                System.out.println("   Balance by Tech Level: " + generateRequest.getBalanceByTechLevel());
                System.out.println("   Balance by Profile: " + generateRequest.getBalanceByProfile());

                MvcResult previewResult = mockMvc.perform(post("/api/lists/{listId}/draws", listId)
                                .param("save", "false")
                ...
                System.out.println("Request: POST /api/lists/" + listId + "/draws?save=true");

                MvcResult saveResult = mockMvc.perform(post("/api/lists/{listId}/draws", listId)
                                .param("save", "true")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(generateRequest)))
        ...
        private GroupPreviewResponse createPreviewResponse() {
                List<PersonResponse> alphaTeam = Arrays.asList(
                                new PersonResponse(UUID.randomUUID(), "Mbappe Killian", "FEMALE", 24, 4, false, 3,
                                                "A_LAISE"),
                                new PersonResponse(UUID.randomUUID(), "Thomas Mular", "MALE", 28, 3, true, 4,
                                                "RESERVE"),
                                new PersonResponse(UUID.randomUUID(), "Luka Modric", "FEMALE", 22, 5, false, 2,
                                                "A_LAISE"));

                List<PersonResponse> betaTeam = Arrays.asList(
                                new PersonResponse(UUID.randomUUID(), "Lucas Hernandez", "MALE", 26, 2, false, 3,
                                                "TIMIDE"),
                                new PersonResponse(UUID.randomUUID(), "Sophie Laurance", "FEMALE", 30, 4, true, 4,
                                                "A_LAISE"),
                                new PersonResponse(UUID.randomUUID(), "Julien Robert", "MALE", 25, 3, false, 2,
                                                "RESERVE"));

                List<PersonResponse> gammaTeam = Arrays.asList(
                                new PersonResponse(UUID.randomUUID(), "Gal Gadot", "FEMALE", 27, 3, false, 4,
                                                "A_LAISE"),
                                new PersonResponse(UUID.randomUUID(), "Antoine Grizman", "MALE", 23, 4, false, 3,
                                                "RESERVE"),
                                new PersonResponse(UUID.randomUUID(), "Pique Girard", "FEMALE", 29, 2, true, 3,
                                                "TIMIDE"));
        ...
        private void displayPreviewResults(MvcResult result) throws Exception {
                ...
                GroupPreviewResponse response = objectMapper.readValue(responseBody, GroupPreviewResponse.class);
                ...
                for (GroupResponse group : response.getGroups()) {
                        System.out.println("\n" + group.getName() + " (" + group.getPersonCount() + " persons)");
                        System.out.println(
        ...
        private void displaySaveResults(MvcResult result) throws Exception {
                String responseBody = result.getResponse().getContentAsString();
                DrawResponse response = objectMapper.readValue(responseBody, DrawResponse.class);
        ...
        // System.out.println("Retrieved " + groups.length + " groups from database:");
